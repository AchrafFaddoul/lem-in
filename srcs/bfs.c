...
/*   Updated: 2019/12/28 22:22:18 by afaddoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_dlist			*ft_flowpath(t_farm *farm, int flow, int i_grp)
{
	t_dlist		*path;
	t_element 	*elm;
	t_element 	*tmp;
...
int 		ft_flowextract(t_farm *farm, t_dlist *vis, int dequeued, int i_grp)
{
	t_dlist 	*path;
	t_item 		*item;
	t_element	*elm;
	...
	farm->grps[i_grp].score = ft_scorecompute(farm->grps[i_grp].path_nb, farm->grps[i_grp].node_nb, farm->ants);
	ft_pathdel(&path);
	i = -1;
	while (++i < farm->grps[i_grp].path_nb)
	{
		...
		{
			if (farm->grps[i_grp].path[j]->size < farm->grps[i_grp].path[i]->size)
			{
...
int 			ft_maxflow(t_farm *farm, int i_grp)
{
	...
	if (!(lst_vis = ft_dlstnew()))
		return (ERROR);
	ft_enqueue(queue, ft_elemnew((t_item*)ft_itemnew(farm->start->index, -1)));
		...
		while (tmp)
		{
			...
			if (!ft_search_item(lst_vis, current_item) && current_item != farm->end->index)
			{
				if (ft_flowvalid(farm, lst_vis, ((t_room*)(tmp->content))->flow, dequeued))
				{
					...
					ft_enqueue(queue, ft_elemnew((t_item*)ft_itemnew(current_item, dequeued)));
				}
			}
			tmp = tmp->next;
		}
